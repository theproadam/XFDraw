           ...
           // Vector3 camRot = new Vector3(0, 0, 0);
            ...
            lock (target.ThreadLock)
            {
                lock (Buffer.ThreadLock)
                {
                    ...
                    WireframeDebug((int*)target.HEAP_ptr, (float*)Buffer.HEAP_ptr, Buffer.Size / divV, stride, color,GetCos(camRot), GetSin(camRot), camPos, RS, &PC, &TC);
        ...
        public static void DrawFlatFill(GLBuffer Buffer, GLTexture target, GLTexture depth, Vector3 camPos, Vector3 camRot, bool mode)
        {
            ...
            lock (target.ThreadLock)
            {
                lock (Buffer.ThreadLock)
                {
                  ...
                  //  if (mode)
                        FillFlatDebug((int*)target.HEAP_ptr, (float*)depth.HEAP_ptr, (float*)Buffer.HEAP_ptr, Buffer.Size / divV, stride, color, GetCos(camRot), GetSin(camRot), camPos, RS, bFCull, & PC, &TC);
                //    else
