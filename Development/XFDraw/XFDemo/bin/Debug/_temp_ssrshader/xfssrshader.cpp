...
inline void FSExec(byte4* FragColor, vec3* nor_data, vec3* pos_data, vec3* norm_data, vec3* frag_pos){
	(*nor_data) = (*norm_data);
	(*pos_data) = (*frag_pos);
...
#pragma endregion
	...
	vec3* ptr_2 = (vec3*)(ptrPtrs[2] + wPos * 12);
			...
			for (int o = FromX; o <= ToX; ++o ){
				...
				FSExec(ptr_0 + o, ptr_1 + o, ptr_2 + o, (vec3*)(attribs + 0), (vec3*)(attribs + 3));
			}
		}
	}
}
