...
inline void shaderMethod(int* color, vec2 viewportMod, vec3 gl_FragCoord){
...
extern "C" __declspec(dllexport) void ShaderCallFunction(long Width, long Height, unsigned char** ptrPtrs, void* UniformPointer){
	vec2 uniform_1;
	fcpy((char*)(&uniform_1), (char*)UniformPointer, 8);
...
#pragma omp parallel for
	for (int h = 0; h < Height; ++h){
		...
		for (int w = 0; w < Width; ++w, ++ptr_0, ++gl_FragCoord.x){
			shaderMethod(ptr_0, uniform_1, gl_FragCoord);
		}
	}
}
