...
inline void VSExec(vec3* pos, vec2* uv, vec3* gl_Position, vec2* uv_data, vec3* frag_pos, vec3 cameraPos, mat3 cameraRot, vec3 objectPos){
...
inline void FSExec(byte4* FragColor, vec2* uv_data, vec3* frag_pos, sampler2D albedoTexture, sampler2D normalTexture, vec2 textureSize, vec3 lightDir){
...
void DrawLineDATA(float* FromDATA, float* ToDATA, float* dptr, float* attrib, char* uData2, unsigned char** ptrPtrs, float zoffset, int Stride, int FaceIndex, int VW, int VH, float farZ)
{
	...
	if (aa * aa > ba * ba)
	{
		...
		for (int i = (int)FromDATA[0]; i <= ToDATA[0]; i++)
		{
			...
			FSExec(ptr_0, (vec2*)(attribs + 0), (vec3*)(attribs + 2), *(sampler2D*)(uData2 + 0), *(sampler2D*)(uData2 + 28), *(vec2*)(uData2 + 56), *(vec3*)(uData2 + 64));}
	}
	else
	{
...
void MethodExec(int index, float* p, float* dptr, char* uData1, char* uData2, unsigned char** ptrPtrs, GLData projData, GLExtra wireData, MSAAConfig* msaa){
	...
	for (int b = 0; b < 3; ++b){
		...
		VSExec((vec3*)(input + 0), (vec2*)(input + 3), (vec3*)(output + 0), (vec2*)(output + 3), (vec3*)(output + 5), *(vec3*)(uData1 + 0), *(mat3*)(uData1 + 12), *(vec3*)(uData1 + 48));
	}
	...
	for (int i = yMin; i <= yMax; ++i)
	{
		if (ScanLinePLUS(i, VERTEX_DATA, BUFFER_SIZE, Intersects, stride, perspMat))
		{
			...
			for (int o = FromX; o <= ToX; ++o ){
				...
				FSExec(ptr_0 + o, (vec2*)(attribs + 0), (vec3*)(attribs + 2), *(sampler2D*)(uData2 + 0), *(sampler2D*)(uData2 + 28), *(vec2*)(uData2 + 56), *(vec3*)(uData2 + 64));
			}
		}
	}
