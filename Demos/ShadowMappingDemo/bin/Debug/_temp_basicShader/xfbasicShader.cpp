...
inline void VSExec(vec3* pos, vec3* norm, vec3* gl_Position, vec3* norm_data, vec3* frag_pos, vec3 cameraPos, mat3 cameraRot, vec3 objectPos){
...
inline void FSExec(byte4* diffuse, vec3* normal, vec3* world_pos, vec3* norm_data, vec3* frag_pos, vec3 objectColor){
...
vec3* ptr_2 = (vec3*)ptrPtrs[2] + mem_addr;
			...
			FSExec(ptr_0, ptr_1, ptr_2, (vec3*)(attribs + 0), (vec3*)(attribs + 3), *(vec3*)(uData2 + 0));}
	}
	else
	{
...
void MethodExec(int index, float* p, float* dptr, char* uData1, char* uData2, unsigned char** ptrPtrs, GLData projData, GLExtra wireData, MSAAConfig* msaa){
	...
	for (int b = 0; b < 3; ++b){
		...
		VSExec((vec3*)(input + 0), (vec3*)(input + 3), (vec3*)(output + 0), (vec3*)(output + 3), (vec3*)(output + 6), *(vec3*)(uData1 + 0), *(mat3*)(uData1 + 12), *(vec3*)(uData1 + 48));
	}
...
#pragma endregion
	...
	vec3* ptr_2 = (vec3*)(ptrPtrs[2] + wPos * 12);
			...
			for (int o = FromX; o <= ToX; ++o ){
				...
				FSExec(ptr_0 + o, ptr_1 + o, ptr_2 + o, (vec3*)(attribs + 0), (vec3*)(attribs + 3), *(vec3*)(uData2 + 0));
			}
		}
	}
